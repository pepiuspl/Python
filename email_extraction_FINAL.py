...
# Paths and constants
THUNDERBIRD_PATH = r"<thunderbird_inbox_path>" #To be changed
SAVE_ROOT = r"<save_root_path>" #To be changed
INTERVAL = 60  # Interval in minutes

# Location for saving email history
HISTORY_FILE_PATH = r"<path_to_history_txt_file>" #To be changed

CLIENT_FOLDER_MAPPING = {
    "<folder_name_in_thunderbird>": "<folder_name_under_save_path>", #To be changed
}
...
def process_email(msg, processed_emails):
    ...
    sender = msg.get("From", "")
    if "<email_address_to_process" not in sender: #To be changed
...
def process_mbox(folder_path, save_root_path, last_received_date, processed_emails):
    ...
    save_path = os.path.join(save_root_path, "<selling_year>", region, client, "<orders_folder>") #To be changed
